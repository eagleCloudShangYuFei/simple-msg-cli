...
async function run(path) {
    const files = await glob(`${path}/**/*.{js,jsx,ts,tsx}`, { ignore: exclude.map(pattern=>`${path}/${pattern}`) });
...
function detectChinese(text, path, type, babelType) {
    if (/[\u4e00-\u9fa5]/.test(text)) {
        console.log('babelType', babelType, text);
...
function report(text, path, type, babelType) {
    ...
    const location = `${path.hub.file.opts.filename}#${loc ? loc.start.line : '!!!'}#${loc ? loc.start.column : '!!!'}`;
